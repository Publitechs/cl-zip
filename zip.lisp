    ...
    (flet ((callback (buffer count)
      ...
      (if (streamp input)
          ...
          (progn
            ...
            (salza2:update crc input 0 (length input))
            (salza2:finish-compression compressor)
            (salza2:reset compressor)
            (values (length input) nout (salza2:result crc)))))))
...
(defun make-zipstream-writer
    (stream &key (external-format (default-external-format)))
  (let ((c (cons nil nil)))
    (make-zipwriter
     :stream stream
     :compressor (make-instance 'salza2:deflate-compressor)
...
(defun close-zipstream-writer (z)
  (write-central-directory z))
